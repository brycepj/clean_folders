...
function email(cfg) {
	...
	io.getGmailPassword().then(function(pw) {
		...
		var mailOptions = {
		    ...
		    subject: count + ' files moved to trash just now.', // Subject line
		    html: html // html body
		};
		// send mail with defined transport object
		transporter.sendMail(mailOptions, function(error, info){
		    if (error){
		        return log.fatal('An error occurred while sending the email.', error);
		    }
		    log.info('Mail sent successfully', info)
		});
